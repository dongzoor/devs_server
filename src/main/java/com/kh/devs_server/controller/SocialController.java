package com.kh.devs_server.controller;

import com.kh.devs_server.dto.SocialDTO;
import com.kh.devs_server.entity.User;
import com.kh.devs_server.service.SocialService;
import lombok.RequiredArgsConstructor;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;
import java.util.HashMap;
import java.util.List;
import java.util.Map;

@RestController
@CrossOrigin
@RequiredArgsConstructor
public class SocialController {
    private SocialService socialService; // Controller 는 넘어온 요청을 처리하기 위해 Service 를 호출한다.

    // social 전체 목록 조회
    @GetMapping("/social")
    public ResponseEntity<List<SocialDTO>> socialList() {
        List<SocialDTO> list = socialService.getSocialList();
        return new ResponseEntity<>(list, HttpStatus.OK);
    }

    // social 디테일 페이지 조회
    @GetMapping("/social/{Id}")  // 이름으로 회원 조회
    public ResponseEntity<SocialDTO> socialList(@PathVariable Long Id) {
        SocialDTO socialDTO = socialService.getSocialList(Id); // getSocialList의 오버로딩
        return new ResponseEntity<>(socialDTO, HttpStatus.OK);
    }

    // social 작성(등록)
    @PostMapping("/social/write")
    public ResponseEntity<Boolean> socialWrite(@RequestBody Map<String, String> regData) { // RequestBody로 받음
        String user = regData.get("user");
        String title = regData.get("title");
        String content = regData.get("content");
        String tag = regData.get("tag");
        String image = regData.get("image");
        boolean result = socialService.regSocial(user, title, content, tag, image);
        if (result) {
            return new ResponseEntity<>(true, HttpStatus.OK);
        } else {
            return new ResponseEntity<>(false, HttpStatus.BAD_REQUEST);
        }
    }

    // social 삭제
    @DeleteMapping("social/{Id}")
    public Map<String, Object> delete(@PathVariable("Id") long Id) {
        Map<String, Object> response = new HashMap<>();
        if (socialService.delSocial(Id) > 0) {
            response.put("result", "SUCCESS");
        } else {
            response.put("result", "FAIL");
            response.put("reason", "일치하는 게시글 정보가 없습니다.");
        }
        return response;
    }
}